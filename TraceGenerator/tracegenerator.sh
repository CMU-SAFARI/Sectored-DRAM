...
if [ -z "$cachefile" ]; then
  cachefile="${TRACEGEN_ROOT}/src/Cache.cfg"
fi
if [ "$force" = true ]; then
...
if [ "$sp" = true ]; then
  if [ "$force" = true ]; then
    ...
    cd ${TRACEGEN_ROOT}/src/PinPoints
    # echo -e "${RED}make isimpoint.test${DEF}"
    make isimpoint.test
    cd $starting
  fi
  echo -e "${RED}${PIN_ROOT}/pin -t ${TRACEGEN_ROOT}/src/PinPoints/obj-intel64/isimpoint.so -slice_size $isize -o $directory/$filename -- $prgname${DEF}"
  ${PIN_ROOT}/pin -t ${TRACEGEN_ROOT}/src/PinPoints/obj-intel64/isimpoint.so -slice_size $isize -o $directory/$filename -- $prgname
  ...
  perl ${TRACEGEN_ROOT}/src/PinPoints/bin/ppgen.3 $directory/$filename $directory/$filename.T.0.bb $directory/$filename.pts$cvgfextension $directory/$filename.w$cvgfextension $directory/$filename.lbl 0
fi
...
# collecting traces
echo -e "${PIN_ROOT}/pin -t ${TRACEGEN_ROOT}/src/obj-intel64/gettrace.so -c $cachefile -intervalsize $isize  $pin_args -ppoints $directory/$filename.1.pp  -- $prgname"
${PIN_ROOT}/pin -t ${TRACEGEN_ROOT}/src/obj-intel64/gettrace.so -intervalsize $isize $pin_args -c $cachefile -ppoints $directory/$filename.1.pp  -- $prgname # TODO: config file needs to change.
