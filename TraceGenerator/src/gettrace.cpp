...
VOID RecordGeneral(VOID * ip, VOID * addr, BOOL isWrite, REG base, REG index, BOOL isfirst, UINT32 size){
  if(record) mem_req++;
  ...
  // TODO: Atb -> make your changes around here
  if(record && (r_check || w_check)) {
    if(strcmp(KnobMode.Value().c_str(),"cpu")==0)   { //Collect CPU traces
      if(!KnobDCEnable.Value() && (!KnobICEnable.Value()|| !KnobIFEnable.Value())) { //unfiltered trace
        if(r_check) {
          trace << std::hex << ip << std::dec << " " << bbl_cnt <<
            std::hex << " R " <<  addr << " " << std::dec << size << std::endl;
          //trace << bbl_cnt << " " << r_addr << " R" << std::endl;
          bbl_cnt=0;
        }
        if(w_check) {
          trace << std::hex << ip << std::dec << " " << bbl_cnt <<
            std::hex << " W " << addr << " " << std::dec << size << std::endl;
