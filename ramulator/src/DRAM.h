...
public:
    ...
    typename T::Command decode(typename T::Command cmd, const int* addr, ulong sectors);
    ...
    bool check_row_hit(typename T::Command cmd, const int* addr, ulong sectors);

    // Check whether a row is open
    bool check_row_open(typename T::Command cmd, const int* addr);
    bool check_sector_miss(typename T::Command cmd, const int* addr, ulong sectors);
    ...
    void update(typename T::Command cmd, const int* addr, long clk, ulong sectors);
    // Update statistics:
...
private:
    ...
    function<typename T::Command(DRAM<T>*, typename T::Command cmd, int, ulong)>* prereq;

    // SAUGATA: added table for row hits
    // Lookup table for whether a command is a row hit
    // E.g., a read command to a closed bank must be preceded by an activate command
    function<bool(DRAM<T>*, typename T::Command cmd, int, ulong)>* rowhit;
    function<bool(DRAM<T>*, typename T::Command cmd, int, ulong)>* sectormiss;
    ...
    function<void(DRAM<T>*, int, ulong)>* lambda;
    ...
    // Helper Functions
    void update_state(typename T::Command cmd, const int* addr, ulong sectors);
    ...
    spec(spec), level(level), id(0), parent(NULL)
{
    ...
    sectormiss = spec->sectormiss[int(level)];
...
// Decode
template <typename T>
typename T::Command DRAM<T>::decode(typename T::Command cmd, const int* addr, ulong sectors)
{
    int child_id = addr[int(level)+1];
    if (prereq[int(cmd)]) {
        typename T::Command prereq_cmd = prereq[int(cmd)](this, cmd, child_id, sectors);
    ...
    return children[child_id]->decode(cmd, addr, sectors);
}
...
// Check row hits
template <typename T>
bool DRAM<T>::check_row_hit(typename T::Command cmd, const int* addr, ulong sectors)
{
    int child_id = addr[int(level)+1];
    if (rowhit[int(cmd)]) {
        return rowhit[int(cmd)](this, cmd, child_id, sectors);  // stop recursion: there is a row hit at this level
    }
    ...
    return children[child_id]->check_row_hit(cmd, addr, sectors);
}

// SAUGATA: added function to check whether a command is a row hit
// Check row hits
template <typename T>
bool DRAM<T>::check_sector_miss(typename T::Command cmd, const int* addr, ulong sectors)
{
    int child_id = addr[int(level)+1];
    if (sectormiss[int(cmd)]) {
        return sectormiss[int(cmd)](this, cmd, child_id, sectors);  // stop recursion: there is a row hit at this level
    }
    ...
    // recursively check for row hits at my child
    return children[child_id]->check_sector_miss(cmd, addr, sectors);
}
...
// Update
template <typename T>
void DRAM<T>::update(typename T::Command cmd, const int* addr, long clk, ulong sectors)
{
    cur_clk = clk;
    update_state(cmd, addr, sectors);
...
// Update (State)
template <typename T>
void DRAM<T>::update_state(typename T::Command cmd, const int* addr, ulong sectors)
{
    int child_id = addr[int(level)+1];
    if (lambda[int(cmd)])
        lambda[int(cmd)](this, child_id, sectors); // update this level

    if (level == spec->scope[int(cmd)] || !children.size())
        return; // stop recursion: updated all levels

    // recursively update my child
    children[child_id]->update_state(cmd, addr, sectors);
}
