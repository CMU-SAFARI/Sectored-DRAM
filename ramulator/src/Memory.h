...
public:
    ...
    void tick()
    {
        ...
        int i = 0;
        for (auto ctrl : ctrls) {
          // For DGMS, we instantiate 8x channels
          // then, to simulate the address bus bottleneck
          // we allow the controller to schedule a command
          // only if another command was not scheduled (approximately)
          bool can_schedule = !DGMS || ((long) num_dram_cycles.value() % 8 == i);
          is_active = is_active || ctrl->is_active();
          ctrl->tick(can_schedule);
          i++;
        }
        if (is_active) {
          ramulator_active_cycles++;
    ...
    bool send(Request req)
    {
        ...
        if (sectoredDRAM && !dynamic_policy)
        {
            //printf("Memory received a new request, sector_bits:%lx addr:%lx\n", req.sector_bits[3], addr);
            assert(req.sector_bits[3] && "Expected non-zero sector bit");
            if (sector_size == 8)
                assert (req.sector_bits[3] < 256 && "Expected sector bits to be smaller");
        }
        ...
        if (use_mapping_file){
            ...
            switch(int(type)){
                ...
                case int(Type::RoBaRaCoCh):
                {
                    ...
                    //int sliced_so_far = addr_bits[0] + addr_bits[addr_bits.size() - 1];
                    for (int i = 1; i <= int(T::Level::Row); i++){
                        req.addr_vec[i] = slice_lower_bits(addr, addr_bits[i]);
                        //sliced_so_far += addr_bits[i];
                        //printf("Level %d, sliced so far: %d\n", i, sliced_so_far);
                    }
                    break;
                }
                default:
        ...
        if (DGMS)
        {
            std::vector<int> channels_required;
            for (int i = 0 ; i < 8 ; i++)
                if ((req.sector_bits[3] >> i) & 0x1)
                    channels_required.push_back(i);

            // Check if we can issue all requests to all channels
            for (auto& ctrl : ctrls)
            {
                if(!ctrl->can_schedule(req.type))
                    return false;
            }

            // Split the request into multiple channels
            for (auto channel : channels_required)
            {
                Request new_req(req);
                new_req.sector_bits[3] = (1 << channel);
                new_req.addr_vec[0] = channel;
                ctrls[channel]->enqueue(new_req);
