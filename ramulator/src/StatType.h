...
class StatList {
 ...
 public:
  ...
  void printall() {
    stat_output << "name,coreid,value" << std::endl;
...
class Stat : public StatBase {
 ...
 public:
  ...
  Derived &coreid(const std::string &__coreid) {
    _coreid = __coreid;
    return self();
  }
...
class VectorBase: public Stat<Derived> {
 ...
 public:
  void init(size_type __size) {
    _size = __size;
    data.resize(size());
    Stat<Derived>::_coreid = "ALL";
    for (off_type i = 0 ; i < size() ; ++i) {
      data[i].flags(0).coreid(std::to_string(i));
      data[i].flags(0).name(Stat<Derived>::_name);
      //data[i].flags(0)
  ...
  void print(std::ofstream& file) {
    ...
    file << Stat<Derived>::_name << "," << Stat<Derived>::_coreid << ",";
    file.precision(Stat<Derived>::_precision);
    //file.width(20);
    //file << std::fixed << total();
    file << std::fixed << total() << std::endl;
    if (size() > 1)
    {
      for (off_type i = 0 ; i < size() ; ++i) {
        data[i].name(Stat<Derived>::_name);
        data[i].print(file);
      }
    }
  }
